...
flowchart TD
    ...
    B --> C[Language / Memory]
    C --> D[Family / Education]
    D --> E[Media / Culture / Algorithm]
    E --> F[Economy / Law / Institutions]
    F --> G[Identity / Avatar]
    G --> H[Consent / Behavior]
    H --> E
```
...
```mermaid
flowchart LR
    P[Problem<br/>tạo hoặc phóng đại crisis] --> R[Reaction<br/>fear, outrage, demand for safety]
    R --> S[Solution<br/>policy/infrastructure đã chuẩn bị]
    S --> C[Consent<br/>public tự xin cái lồng mới]
```
...
```mermaid
flowchart TD
    A[Unthinkable] --> B[Controversial]
    B --> C[Debated]
    C --> D[Represented in media]
    D --> E[Policy pilot]
    E --> F[Normalized]
    F --> G[Mandatory / default]
```
...
```mermaid
flowchart LR
    A[See the Frame] --> B[Withdraw Automatic Consent]
    B --> C[Reclaim Body / Attention]
    C --> D[Rebuild Sovereignty]
    D --> E[Remember Monad]
    E --> F[Act From Gnosis]
```
