...
OUT=ROOT/'_docs'/'theravada-batch5-scripture-audit.json'
cache={}
def fetch(rid):
 if rid not in cache:
...
for n in range(29,33):
 ...
 while i<len(lines):
  m=re.match(r'> \*\*Pāli — ([A-Za-z]{2,})\s*(\d+(?:\.\d+)?), .*?`(.+?)`\*\*$',lines[i])
  ...
  i=j+1
report={'schema':'theravada-scripture-audit.v1','scope':'lessons-29-32','blocks':len(rows),'passed':sum(x['exact_match'] for x in rows),'all_pass':all(x['exact_match'] for x in rows),'rows':rows}
