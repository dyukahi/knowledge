...
```mermaid
graph LR
    subgraph "3 NGUỒN KÍCH HOẠT / 3 TRIGGERS"
        A[Aggression] --> D[Schadenfreude]
        B[Rivalry] --> D
        C[Justice] --> D
    end
...
```mermaid
flowchart LR
    A[Ai đó sai lầm / Someone messes up] --> B[Viral outrage]
    B --> C[Pile-on / Everyone jumps in]
    C --> D[Collective Schadenfreude]
    D --> E[Dopamine cho tất cả / Dopamine for everyone]
    E --> F[Engagement cao / High engagement]
    F --> G[Algorithm đẩy lên / Algorithm boosts]
    G --> B
```
...
```mermaid
graph TD
    A[Chia phe: Ta vs Chúng Nó / Divide: Us vs Them] --> B[Out-group dehumanization]
    B --> C[Schadenfreude khi phe kia khổ / when other side suffers]
