...
status: draft
aliases: ["Em Bé Bắp Cải", "Cabbage Patch Baby", "Cabbage Patch Kids Symbolism", "Đứa Trẻ Mọc Từ Ruộng"]
related:
...
flowchart LR
    A[Womb / Mother body]
    B[Lineage / Blood memory]
    C[Family tree]
    D[Cabbage patch / Field]
    E[Harvest]
    F[Delivery / Adoption]
    A --> B --> C
    D --> E --> F
...
```mermaid
flowchart TD
    A[Folklore: baby from cabbage]
    B[Toy ritual: adoptable baby + certificate]
    C[Media: manufactured / selected children]
    D[Clinic: embryo selection / surrogacy]
    E[Future: artificial womb infrastructure]
    F[System: child as managed identity]
    A --> B --> C --> D --> E --> F
```
