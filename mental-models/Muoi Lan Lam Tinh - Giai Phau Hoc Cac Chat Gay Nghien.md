...
status: refined
related:
  - "[[Dopamine Economy - Nen Kinh Te Cua Su Them Muon]]"
  - "[[Loosh - Nang Luong Thu Hoach Tu Con Nguoi]]"
  - "[[S.E.X Va Tam Ly Hoc Jung]]"
  - "[[Kiểm Soát Tâm Trí]]"
  - "[[Sacred Plants Corrupted - Thuoc La, Ruou va Cu Lua The Ky]]"
  - "[[Ma Trận]]"
  - "[[Elite]]"
...
```mermaid
flowchart LR
    subgraph "Bình thường / Normal"
        A1[Neuron A] -->|"release"| S1[Synapse]
        S1 -->|"bind"| B1[Neuron B]
        S1 -.->|"reuptake"| A1
    end
    
    subgraph "Khi dùng Meth / On Meth"
        A2[Neuron A] -->|"release + reverse pump"| S2[Synapse FLOODED]
        S2 -->|"continuous stimulation"| B2[Neuron B]
        A2 -.->|"DAT blocked & reversed"| S2
    end
```
...
flowchart TD
    M[Meth] --> D[Dopamine ↑↑↑]
    M --> N[Norepinephrine ↑↑]
    D --> W[Ham muốn tâm lý CAO]
    N --> V[Co mạch ngoại vi]
    V --> E[Khó cương / khó thực hiện]
    W --> P[Muốn nhưng không thể]
    E --> P
```
...
flowchart LR
    subgraph "Cocaine"
        C[Cocaine] --> BD[Block DAT]
        C --> BN[Block NET]
        C --> BS[Block SERT]
    end
    
    subgraph "Meth"
        M[Meth] --> RD[Reverse DAT]
        M --> RN[Reverse NET]
    end
    
    BD --> S1[Spike nhanh, ngắn<br/>20-30 min]
    RD --> S2[Spike cao, dài<br/>8-12 hours]
```
...
Xem thêm: [[Sacred Plants Corrupted - Thuoc La, Ruou va Cu Lua The Ky|Sacred Plants Corrupted]]
...
flowchart TD
    MDMA --> S[Serotonin ↑↑↑↑]
    MDMA --> D[Dopamine ↑↑]
    MDMA --> N[Norepinephrine ↑↑]
    
    S --> Safe[Cảm giác an toàn]
    S --> Trust[Tin tưởng]
    S --> Connect[Kết nối]
    S --> Amyg[Ức chế Amygdala]
    
    Amyg --> NoThreat[Không còn threat detection]
    NoThreat --> Friends[Mọi người = bạn tiềm năng]
```
...
```mermaid
flowchart LR
    Sat[Thứ 7: MDMA] --> Sun[CN: Afterglow]
    Sun --> Mon[T2: Okay]
    Mon --> Tue[T3: CRASH]
    Tue --> Wed[T4-5: Depression]
    Wed --> Week[1-2 tuần: Recovery]
```
...
```mermaid
flowchart TD
    LSD --> DMN[Ức chế Default Mode Network]
    LSD --> Connect[Tăng kết nối giữa các vùng não]
    
    DMN --> EgoD[Ego dissolution]
    Connect --> Syn[Synesthesia: nghe màu, thấy âm]
    
    EgoD --> Merge[Hòa tan vào môi trường]
    Syn --> Deep[Đối mặt nội dung tâm lý sâu]
```
...
```mermaid
flowchart TD
    subgraph "Các chất khác / Other drugs"
        M[Meth] --> G1[Muốn làm việc nhiều hơn]
        C[Cocaine] --> G2[Muốn thắng nhiều hơn]
        MDMA --> G3[Muốn kết nối nhiều hơn]
        LSD --> G4[Muốn hiểu nhiều hơn]
    end
    
    subgraph "Heroin"
        H[Heroin] --> G5[Muốn ngừng cảm thấy]
        G5 --> D[Đã từ bỏ cuộc tìm kiếm]
    end
```
...
```mermaid
flowchart TD
    subgraph "Châu Á / Asia"
        MA[Meth] --> WA[Work harder, survive]
        KA[Ketamine] --> EA[Escape trapped life]
    end
    
    subgraph "Americas"
        CA[Cocaine] --> WC[Win, dominate]
        HA[Heroin] --> GU[Given up]
    end
    
    subgraph "Châu Âu / Europe"
        ME[MDMA] --> CO[Connect, belong]
        LE[LSD] --> UN[Understand, transcend]
    end
    
    subgraph "Toàn cầu / Global"
        CG[Cannabis] --> AN[Reduce anxiety]
        AL[Alcohol] --> LE2[Legal escape]
    end
```
...
- [[Sacred Plants Corrupted - Thuoc La, Ruou va Cu Lua The Ky]] — Cây thiêng bị biến chất
