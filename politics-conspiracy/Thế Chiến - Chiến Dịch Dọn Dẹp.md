...
aliases: ["Thế Chiến - Chiến Dịch Dọn Dẹp"]
...
```mermaid
flowchart TD
    subgraph SURFACE["BỀ MẶT / SURFACE"]
        A["Axis vs Allies"]
        B["Democracy vs Fascism"]
        C["Good vs Evil"]
    end

    subgraph REALITY["THỰC TẾ / REALITY"]
        D["Cùng một Elite"]
        E["Cùng một mục đích"]
        F["Cùng một kết quả"]
    end

    SURFACE --> MASK["NHỊ NGUYÊN<br>= VỎ BỌC"]
    MASK --> REALITY

    REALITY --> G["Phá hủy đế chế cũ<br>Destroy old empires"]
    REALITY --> H["Xóa kiến trúc cổ<br>Erase old architecture"]
    REALITY --> I["Viết lại lịch sử<br>Rewrite history"]
    REALITY --> J["Dọn dẹp công nghệ cũ<br>Cleanup old tech"]
```
...
```mermaid
flowchart LR
    WW1["WW1<br>1914-1918"] --> A["Ottoman Empire<br>Collapsed"]
    WW1 --> B["Austro-Hungarian<br>Dissolved"]
    WW1 --> C["Russian Tsardom<br>Bolshevik Revolution"]
    WW1 --> D["German Empire<br>Weimar Republic"]

    A --> E["Middle East Redrawn"]
    B --> F["Central Europe Redrawn"]
    C --> G["Soviet Union Created"]
    D --> H["Set up for WW2"]
```
...
```mermaid
flowchart TD
    subgraph GLOBAL["STAR FORTS WORLDWIDE"]
        EU["Europe<br>Highest concentration"]
        AS["Asia<br>Vietnam, Japan, India, Indonesia"]
        AM["Americas<br>USA, Brazil, Caribbean"]
        AF["Africa<br>North & coastal"]
    end
    ...
    Q1 --> Q2["Same time period?"]
    Q2 --> Q3["Same builders?"]
    Q3 --> Q4["Same civilization?"]
```
...
```mermaid
flowchart TD
    A["Pre-WW1: Old World Intact"] --> B["WW1 (1914-1918)<br>4 empires collapse"]
    B --> C["Interwar: World's Fairs<br>Systematic demolition"]
    C --> D["WW2 (1939-1945)<br>Historic cities destroyed"]
    D --> E["Post-WW2: UN, NATO<br>New World Order"]
    E --> F["1945+: Paperclip<br>Nazi scientists absorbed"]
    F --> G["Cold War: Vietnam, Korea<br>More 'cleanup'"]
    G --> H["Today: History = Standardized"]
```
...
```mermaid
flowchart TD
    WW["World Wars"] --> A["Old empires destroyed"]
    WW --> B["Historic evidence destroyed"]
    WW --> C["Populations reduced"]
    WW --> D["Fear trauma installed"]

    A --> E["Elite consolidates power"]
    B --> E
    C --> E
    D --> E

    E --> F["New World Order<br>UN, IMF, World Bank"]
```
