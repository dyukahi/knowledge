...
```mermaid
flowchart TD
    A[Capital / Ownership] --> B[Think Tanks / Foundations]
    B --> C[Policy / Regulation]
    C --> D[Media / Academia]
    D --> E[Public Opinion]
    E --> F[Consent]
    F --> G[Infrastructure Change]
    G --> A
```
...
```mermaid
flowchart LR
    A[Crisis / Fear] --> B[Demand For Protection]
    B --> C[Emergency Powers]
    C --> D[New Infrastructure]
    D --> E[Temporary Becomes Permanent]
```
